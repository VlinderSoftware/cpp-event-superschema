...
TEST_CASE("Event dispatcher", "[dispatcher]") {
    SECTION("Dispatches valid event to correct handler") {
        ...
        handlers["test.event"] = [&handler_called, &received_event](
            const ErrorHandler& err, const json& event
        ) {
            handler_called = true;
    ...
    SECTION("Calls error handler for invalid event") {
        ...
        EventHandlers handlers;
        handlers["test.event"] = [&handler_called](
            const ErrorHandler& err, const json& event
        ) {
            handler_called = true;
            (void)err;
    ...
    SECTION("Dispatches to default handler when no specific handler found") {
        ...
        EventHandlers handlers;
        handlers["__default__"] = [&default_handler_called, &received_event](
            const ErrorHandler& err, const json& event
        ) {
    ...
    SECTION("Does nothing when no handler found and no default") {
        ...
        EventHandlers handlers;
        handlers["other.event"] = [&any_handler_called](
            const ErrorHandler& err, const json& event
        ) {
